...
**Stable at v1.0.0. Python-only. Phases 0–3 complete.**

- Phase 0 gate passed: every corpus document round-trips byte-stable with zero diff.
- Phase 1 surface implemented and green, including the MCP stdio server.
...
- Phase 3: hostile-input hardening — zip-bomb caps (count/size/ratio), `<!DOCTYPE`/`<!ENTITY` rejection (XXE / billion-laughs), XML depth caps, path-traversal clamping — all tunable via `DOCXENGINE_MAX_*` and covered by adversarial suites + conformance cases (algorithms.md §27). Added the large-document perf benchmark (`bench/perf.py`) and the renderer-fidelity harness + protocol (`conformance/fidelity/`, `docs/conformance/fidelity.md`).
- Totals: 476 Python tests, agent-benchmark tasks over MCP (zero tool errors, zero Word-repair events).
...
- Conformance corpus + harness wired up.

**Gate**: every corpus document round-trips with zero diff and zero Word-repair prompts.
...
- MCP server (stdio) + Python package + OpenAI/Anthropic function-calling adapters.
...
**Gate**: full surface green on the expanded conformance corpus; benchmark coverage for table/style/comment/template tasks.
