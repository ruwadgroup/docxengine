...
- **`docx_insert` markdown** — `content` splits on `\n` (one trailing `\r` stripped per line); lines that are empty/whitespace-only emit nothing. Each remaining line is classified with the **§22 line grammar** (the shared `classifyLine`/`classify_line`), minus multi-line tables: `#{1..6}` + space → `pStyle Heading{n}`; `>` → `pStyle Quote`; `---`/`***` alone → the §22 horizontal-rule paragraph (`w:pBdr`, no runs); `- [ ]`/`- [x]` (or `*`) → `pStyle ListParagraph` with the text prefixed by the §22 checkbox glyph (`☐ `/`☒ `); `- `/`* `/`1. ` list item → `pStyle ListParagraph` (`numPr` wiring is Phase 2); anything else → plain paragraph. **Inline** markdown (`**`/`__`, `*`/`_`, `` ` ``, `<br>`) is parsed into runs per §22 (`emitInline`), so each paragraph may emit multiple runs. A `style` argument overrides every inserted paragraph's `pStyle`: the styleId is the argument verbatim if defined in `word/styles.xml`, else the argument with whitespace removed if defined, else `style_unknown`; under an override a `---` rule becomes a styled empty paragraph. With tracking, each inserted paragraph's run sequence is wrapped in one `w:ins` (§5 ids in document order; an empty/rule paragraph is not wrapped). Exactly one of `after`/`before` must be given (`anchor_invalid`).
...
**style.** Without `props`, the legacy form stamps the `TableGrid` named style (`<w:tblStyle w:val="TableGrid"/>`). With `props`, apply **direct** formatting by rebuilding `w:tblPr` in canonical child order — `tblStyle?`, `tblW`, `jc?`, `tblBorders?`, `tblLayout?`, `tblLook?` (preserving any existing `tblStyle`/`tblLook`): `borders:false` → all six `w:tblBorders` sides `w:val="none"`; `borders:true` → `w:val="single" w:sz="{border_weight|4}" w:space="0" w:color="{border_color|auto}"` (hex color, leading `#` stripped); `width_pct:n` → `<w:tblW w:w="{round(n*50)}" w:type="pct"/>` (else `w:w="0" w:type="auto"`); `align` → `<w:jc w:val="{left|center|right}"/>`; `col_widths:[…]` (twips) → rewrite `w:tblGrid` and add `<w:tblLayout w:type="fixed"/>`; `shading:{header|all: hex|null}` → set/remove `<w:shd w:val="clear" w:color="auto" w:fill="{hex}"/>` in each targeted cell's `w:tcPr` (header = the first `w:tr`; `header` overrides `all` on the first row; `null` removes the band). Edits are non-overlapping and applied in one splice. **delete.** Remove the whole `w:tbl` span for the `T#` anchor; there is no tracked-change form (OOXML has no table-level `w:del`).
...
Resolution order for `soffice`: env `DOCXENGINE_SOFFICE`; then `soffice` on `PATH`; then platform defaults `/Applications/LibreOffice.app/Contents/MacOS/soffice`, `/usr/bin/soffice`; then a previously **auto-fetched** build in the cache; then, when nothing is found and auto-fetch is enabled (default; off via `DOCXENGINE_AUTO_FETCH_SOFFICE=0`), an official LibreOffice build is **downloaded on demand** from The Document Foundation over HTTPS and **SHA-256-verified against the publisher's `.sha256` sidecar** before use, then cached under `DOCXENGINE_SOFFICE_CACHE` (default `$XDG_CACHE_HOME/docxengine`). Auto-fetch covers macOS (`.dmg`) and Linux x86-64 (`.deb` tarball); other platforms fall back to detection. Invocation: `soffice --headless --convert-to pdf --outdir {DIR} {FILE}` with a **per-call temp profile** dir via `-env:UserInstallation=file://{tmp}` (cleaned up after). On success `renderer` = `"libreoffice {version}"` and each `pages[]` entry carries an `image` resource link. When no binary is available (auto-fetch disabled, unsupported, or failed), the **structural fallback**: `renderer:"structural"`, the `structural` §2 projection, and `page_count` = the estimated page count (`ceil(total_chars / 1800)`) — **no `pages[]` array** (no renderer ran, so per-page image links would be misleading, and an array of bare `{page}` objects carries no information beyond the count); the `note` explains why (auto-fetch off/failed) and how to enable rendering. `docx_render_preview` never errors; `docx_convert` to pdf/png with no adapter is `render_unavailable`. PNG via PDF then `pdftoppm` (or macOS `sips`) when available, else the structural fallback.

## 25. MCP Streamable HTTP + resources (Python server only)

`docxengine-mcp --http --port {p}` runs a stdlib `http.server` (threading). **POST /** accepts a JSON-RPC body (same dispatch as stdio), responds `application/json`. `initialize` allocates an `Mcp-Session-Id` (returned as a response header); every subsequent POST must send it (missing/unknown → JSON-RPC error; an expired session → HTTP **410**). Sessions are **protocol-only** — document state is the filesystem, not the session, so there is no per-session doc store. **GET /health** → `200` (body `{"status":"ok"}`). **resources/list** is empty (the filesystem is not enumerated); **resources/templates/list** advertises `docx://{path}/outline` and `docx://{path}/projection`, and **resources/read** renders either on demand from the file at `{path}` (`outline` = §2a outline rendering, `projection` = the full §2 projection). stdio mode shares the same dispatch and remains the conformance transport. Both transports project the doc_id contract through the §26 file facade.
