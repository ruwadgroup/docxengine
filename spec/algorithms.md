...
Detection order: env `DOCXENGINE_SOFFICE`; then `soffice` on `PATH`; then platform defaults `/Applications/LibreOffice.app/Contents/MacOS/soffice`, `/usr/bin/soffice`. Invocation: `soffice --headless --convert-to pdf --outdir {DIR} {FILE}` with a **per-call temp profile** dir via `-env:UserInstallation=file://{tmp}` (cleaned up after). On success `renderer` = `"libreoffice {version}"` and each `pages[]` entry carries an `image` resource link. When no binary is found, the **structural fallback**: `renderer:"structural"`, the `structural` §2 projection, and `page_count` = the estimated page count (`ceil(total_chars / 1800)`) — **no `pages[]` array** (no renderer ran, so per-page image links would be misleading, and an array of bare `{page}` objects carries no information beyond the count); the `note` states the LibreOffice/`DOCXENGINE_SOFFICE` requirement. `docx_render_preview` never errors; `docx_convert` to pdf/png with no adapter is `render_unavailable`. PNG via PDF then `pdftoppm` (or macOS `sips`) when available, else the structural fallback.
