...
Every tool takes a file **`path`** instead of an in-memory handle: each call opens the file, runs the tool, and — when the edit changed it — validates and saves it back atomically. There is no `doc_id` and no separate save step; `docx_create` writes its file immediately. `tools/list` is generated by transforming the same JSON Schemas in [`spec/`](../../spec/) that feed the SDKs (`doc_id` → `path`, `docx_save` dropped — 23 tools), never hand-maintained.

The SDKs keep the `doc_id`/bytes contract unchanged — it is the right storage-agnostic surface for embedding in software, including browser JS with no filesystem. The path surface is specific to this server. See the [tool reference](../tools/index.md).
